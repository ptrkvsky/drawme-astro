...
const getTranslationDistance = (element1, spread = 400, maxDistance = 5000) => {
  ...
  spread = Math.max(
    map(getDistanceToCenter(element1), 0, maxDistance, spread, 0),
    0
  );

  const angle = Math.atan2(
    Math.abs(win.y - elCenter.y),
    Math.abs(win.x - elCenter.x)
  );
...
export { preloadImages, map, getTranslationDistance, getDistanceToCenter };
