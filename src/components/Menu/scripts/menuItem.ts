...
export default class MenuItem {
  DOM: {
    ...
    revealImage: HTMLDivElement;
    el: Element;
  };
  inMenuPosition: number;
  animatableProperties: AnimatableProperties;
  bounds: { el: DOMRect; reveal: any };
  mouseenterFn: (ev: any) => void;
  firstRAFCycle: boolean;
  mouseleaveFn: () => void;
  tl: gsap.core.Timeline;
  requestId: any;

  constructor(el: Element, inMenuPosition: number, animatableProperties: any) {
    // el is the <a> with class "menu__item"

    this.DOM = {
      el: el,
      textInner: document.querySelector('.menu__item-textinner'),
    ...
    this.tl = gsap
      .timeline({
        onStart: () => {
          ...
          gsap.set(this.DOM.el, { zIndex: 800 });
        },
      })
